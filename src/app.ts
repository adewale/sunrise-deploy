...
function renderItem(item: GitHubActionItem) {
  const chips = [`Type: ${item.kind.replaceAll('_', ' ')}`, item.repo, item.source, new Date(item.updatedAt).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'UTC' })].map((chip) => `<span class="chip">${escapeHtml(chip)}</span>`).join('');
