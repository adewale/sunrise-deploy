...
function renderItem(item: GitHubActionItem, ownerLogin = '') {
  ...
  const check = item.evidence?.checks ? `<span class="check-status ${escapeHtml(item.evidence.checks)}">${escapeHtml(item.evidence.checks)}</span>` : '';
  return `<article class="item"><time class="item-time" datetime="${escapeHtml(item.updatedAt)}"><span>${escapeHtml(when.date)}</span><strong>${escapeHtml(when.time)}</strong></time><div class="item-main"><div class="item-topline"><span class="type-icon" aria-hidden="true">${escapeHtml(itemIcon(item))}</span><img class="repo-avatar" src="https://github.com/${escapeHtml(repoOwner)}.png?size=40" alt="" loading="lazy">${authorAvatar}<div class="chips">${chips}${check}</div></div><a class="item-title" href="${item.url}">${escapeHtml(item.title)}</a><p>${escapeHtml(item.reason)}</p><p class="action">${escapeHtml(item.suggestedAction)} <span class="relative-time">· updated ${escapeHtml(relativeTime(item.updatedAt))}</span></p></div></article>`;
}
