...
function renderItem(item: GitHubActionItem) {
  ...
  return `<article class="item"><time class="item-time" datetime="${escapeHtml(item.updatedAt)}"><span>${escapeHtml(when.date)}</span><strong>${escapeHtml(when.time)}</strong></time><div class="item-main"><div class="chips">${chips}</div><a class="item-title" href="${item.url}">${escapeHtml(item.title)}</a><p>${escapeHtml(item.reason)}</p><p class="action">${escapeHtml(item.suggestedAction)}</p></div><form method="post" action="/ignore"><input type="hidden" name="canonicalSubjectKey" value="${escapeHtml(item.canonicalSubjectKey)}"><button class="button quiet">Ignore</button></form></article>`;
}
