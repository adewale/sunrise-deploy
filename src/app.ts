...
function renderUnresolvedLinks(rows: Array<{ label: string; count: number; href: string; query?: string }> = []) {
  if (!rows.length) return '<p class="empty compact">All clear.</p>';
  return rows.map((row) => `<a class="stat unresolved-link" href="${escapeHtml(row.href)}" target="_blank" rel="noreferrer" title="${escapeHtml(row.query ? `Opens GitHub: ${row.query}` : 'Opens GitHub')}"><span>${escapeHtml(row.label)}</span><strong>${row.count} ↗</strong></a>`).join('');
}
